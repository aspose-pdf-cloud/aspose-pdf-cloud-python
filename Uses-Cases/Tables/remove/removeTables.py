...
class PdfTables:
        ...
        if not self.pdf_api:
            ...
            if resultTabs.code == 200 and resultTabs.tables:
                if not resultTabs.tables.list or len(resultTabs.tables.list) == 0:
                    logging.error("Unexpected error : tables is null or empty!!!")
    ...
    def delete_table(self):
        if not self.pdf_api:
            ...
            if resultTabs.code == 200:
                logging.info(f"Table #{Config.TABLE_ID} deleted!")
            else:
                logging.error("Unexpected error : can't delete table!")
    ...
    def delete_tables(self):
        if not self.pdf_api:
            ...
            if resultTabs.code == 200:
                logging.info(f"Tables on page #{Config.PAGE_NUMBER} deleted!")
            else:
                raise TypeError("Unexpected error : can't get tables!!!")
